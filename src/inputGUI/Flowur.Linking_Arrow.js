...
var Linking_Arrow = new Class({
	initialize: function(x,y,t,f,lnode){
		...
		this.setup_obs = function(){
			...
			b.obs_box_top = paper.rect(b.x + (b.width/2 - (obs_width-obs_offset)/2), b.y - (b.height+obs_offset) - (obs_height-obs_offset)/2, (obs_width-obs_offset), (obs_height-obs_offset), obs_radius).attr({fill: '#DADADA', stroke: 'none', 'opacity': 1});
			
			b.obs_node_back_box = paper.rect(n_x, n_y +(node_offset*scale), n_width, n_height, scale*node_radius).attr({'fill': n_color, 'stroke': 'none'});
			
			b.obs_node_back_shader = b.obs_node_back_box.clone();
			b.obs_node_back_shader.attr({'fill': '#000000', 'opacity': .45});
			
			b.obs_node_top_box = paper.rect(n_x+(node_offset*scale), n_y , n_width, n_height, scale*node_radius).attr({'fill': n_color, 'stroke': 'none'});
		...
		this.setup_obs();
		this.top_graphic = paper.path('M'+this.x+','+(this.y+this.head_height)+'l'+(this.head_width/2)+','+(-this.head_height)+'l'+(this.head_width/2)+','+(this.head_height)+'h'+(-((this.head_width/2)-(this.thickness/2)))+'v'+(this.height-this.head_height)+'h'+(-this.thickness)+'v'+(-(this.height-this.head_height))+'h'+(-((this.head_width/2)-(this.thickness/2)))).attr({fill: '#999999', stroke: 'none'});
	...
	set_x: function(new_x){
		...
		this.back_graphic.translate(-.4,.8);
		this.top_graphic.toFront();
		this.striker_box.attr({'x': this.x});
		this.striker_box.toFront();
		///////////////////////////////////////
		var scale = .7;
